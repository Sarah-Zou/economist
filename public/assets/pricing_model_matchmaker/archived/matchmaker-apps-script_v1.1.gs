...
function doGet(e) {
  ...
  if (hostReady && htmlStr) {
    ...
    return HtmlService.createHtmlOutput(htmlStr).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.SAMEORIGIN);
  }
  ...
  return HtmlService.createHtmlOutput(pendingHtml).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.SAMEORIGIN);
}
...
function processPolishedReports_() {
  ...
  try {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (!reportHtml && !reportFileId) continue;
...
function getProductContext_(productUrl) {
  ...
  try {
    ...
    out.title = stripHtml_(firstMatch_(html, /<title[^>]*>([\s\S]*?)<\/title>/i)).slice(0, 200);
...
/**
 * Prepares stored HTML for serving safely:
 * - strips scripts + risky attributes
 * - injects noindex/nofollow
 * - injects a CSP that blocks scripts
 */
function prepareReportHtmlForServing_(html) {
  var out = String(html || "");
  out = sanitizeReportHtml_(out);
  out = ensureRobotsNoIndex_(out);
  out = ensureCspNoScript_(out);
  return out;
}

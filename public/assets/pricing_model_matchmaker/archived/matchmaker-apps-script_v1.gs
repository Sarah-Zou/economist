...
function doGet(e) {
  ...
  var statusCol = SHEET_COLUMNS.indexOf("report_status") + 1;
  var status = String(sheet.getRange(rowIndex, statusCol).getValue() || "").trim();
...
function processPolishedReports_() {
  ...
  try {
    ...
    var width = SHEET_COLUMNS.length;
    var values = sheet.getRange(2, 1, lastRow - 1, width).getValues();

    var statusIdx = SHEET_COLUMNS.indexOf("report_status");
    var emailIdx = SHEET_COLUMNS.indexOf("email");
    var reportUrlIdx = SHEET_COLUMNS.indexOf("report_url");
    var reportHtmlIdx = SHEET_COLUMNS.indexOf("report_final_html");
    var lastErrorIdx = SHEET_COLUMNS.indexOf("last_error");
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      var reportHtml = String(values[i][reportHtmlIdx] || "").trim();

      if (!email || !reportUrl || !reportHtml) continue;
